...
class WMT14DataModule(LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        train_dataset = WMT14Dataset(self.langpair, self.source_train, self.target_train)
        return DataLoader(train_dataset, batch_size=self.model_config.train_hparams.batch_size, shuffle=True, drop_last=True, num_workers=-1)

    def valid_dataloader(self) -> DataLoader:
        val_dataset = WMT14Dataset(self.langpair, self.source_val, self.target_val)
        return DataLoader(val_dataset, batch_size=self.model_config.train_hparams.batch_size, shuffle=False, drop_last=False, num_workers=-1)

    def test_dataloader(self) -> DataLoader:
        test_dataset = WMT14Dataset(self.langpair, self.source_test, self.target_test)
        return DataLoader(test_dataset, batch_size=self.model_config.train_hparams.batch_size, shuffle=False, drop_last=False, num_workers=-1)
